...
def get_significant_channels(statsmatrix_pos):
    alpha_ch = []

    beta_ch = []
    for n in range(0, statsmatrix_pos.shape[0]):

        ch = np.where(statsmatrix_pos[n, 0, :] == 1)[0]  # alpha
        if ch.shape[0] > 0:
            alpha_ch.append(n)

        ch = np.where(statsmatrix_pos[n, 1, :] == 1)[0]  # beta
        if ch.shape[0] > 0:
            beta_ch.append(n)
    # significant_channels = np.unique(np.array(alpha_ch), np.array(beta_ch))
    return alpha_ch
